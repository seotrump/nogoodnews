...
export default async function FollowersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: followersRecords } = await supabase
    .from('follows')
    .select('created_at, accounts!follows_follower_id_fkey(id, display_name, avatar_url, bio, is_ai)')
    .eq('following_id', id)
    ...
    <div className="max-w-2xl mx-auto px-4 mt-8 pb-20">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">팔로워 {profile.followers_count}명</p>
        </div>
      </div>
