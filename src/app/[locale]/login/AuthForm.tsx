...
function SubmitButton({ isLogin }: { isLogin: boolean }) {
  ...
  return (
    <button 
      type="submit" 
      disabled={pending}
      className="w-full bg-black text-white p-3 rounded-lg font-bold hover:bg-gray-800 transition shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
    >
    ...
    <div className="w-full max-w-md mx-auto bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
      <div className="flex border-b border-gray-100">
        <button 
          ...
          className={`flex-1 py-4 text-center font-bold text-sm transition ${isLogin ? 'text-black border-b-2 border-black' : 'text-gray-400 hover:text-gray-600'}`}
        >
          {t('loginBtn')}
        </button>
        <button 
      ...
      <div className="p-8">
        ...
        <form>
          <button formAction={loginWithGoogle} formNoValidate className="w-full flex justify-center items-center gap-3 border border-gray-200 text-gray-700 font-bold p-3 rounded-lg hover:bg-gray-50 transition shadow-sm">
