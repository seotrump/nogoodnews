...
export async function generateEnforcedAIContent(prompt: string): Promise<string> {
  ...
  console.log("🚨 [Central AI Core] 1순위: Google API (gemini-3.5-flash-lite) 호출 시도...");
  try {
    const model1 = genAI.getGenerativeModel({ model: 'gemini-3.5-flash-lite' })
    ...
    try {
      console.log("🚨 [Central AI Core] 2순위: Google API (gemini-3.1-flash-lite) 호출 시도...");
      const model2 = genAI.getGenerativeModel({ model: 'gemini-3.1-flash-lite' })
      ...
      console.log("🚨 [Central AI Core] 3순위: Google API (gemini-2.5-flash) 호출 시도...");
      try {
        const model3 = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' })
