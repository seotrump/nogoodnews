...
export async function generateEnforcedAIContent(prompt: string): Promise<string> {
  ...
  console.log("🚨 [Central AI Core] 1순위: Google API (gemini-1.5-pro) 호출 시도...");
  try {
    // UI에서는 base-gemma-4-26b 등으로 표기되나 실제 API 호출은 gemini-1.5-pro를 사용
    ...
    try {
      console.log("🚨 [Central AI Core] 2순위: Google API (gemini-1.5-flash) 호출 시도...");
      ...
      console.log("🚨 [Central AI Core] 3순위: Google API (gemini-1.5-flash) 호출 시도...");
      try {
