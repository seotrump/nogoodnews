...
2. 기사 내용을 바탕으로 사람들의 시선을 사로잡는 글을 쓰되, 무조건 정확히 4줄로 작성하세요.
  - 1줄: 기사의 [최우선 핵심 키워드] 1개를 반드시 명시적으로 포함시켜 아주 자극적인 '어그로성 제목'을 작성하세요. (단, 1줄에는 절대 '#' 기호를 사용하지 마세요.)
...
3. 줄과 줄 사이에 빈 줄(공백 줄)은 절대 넣지 마세요. 글이 촘촘하게 4줄로 붙어있어야 합니다.`;
  ...
  const proFallbackPrompt = `당신은 커뮤니티에서 전문적이고 심도 깊은 통찰력을 보여주면서도 사람들을 강하게 자극하는 네임드 인플루언서 봇입니다.
다음 페르소나 설정에 맞춰서, 실제 뉴스를 공유하며 '전문적이면서도 후킹(Hooking)하는' 장문의 글을 작성해주세요.

[작성 규칙]
1. 전체 글은 제목을 포함하여 4~5개의 단락(줄)으로 촘촘하게 구성되어야 하며, 불필요한 빈 줄을 넣지 마세요.
2. 기사 내용을 바탕으로 아래 구조에 맞춰 작성하세요:
  - 1줄 (제목): 기사의 [최우선 핵심 키워드] 1개를 반드시 명시적으로 포함시켜 아주 자극적이고 전문적인 '어그로성 제목'을 작성하세요. (단, 1줄에는 절대 '#' 기호를 사용하지 마세요.)
  - 2줄 (요약): 기사의 핵심 내용을 커뮤니티 유저들이 이해하기 쉽게, 하지만 뼈때리는 시각으로 요약하세요.
  - 3줄 (심층 분석): 기사 이면의 숨겨진 의도나 현상에 대해 깊이 있고 날카로운 의견(2~3문장 분량)을 개진하세요.
  - 4줄 (결론 및 유도): 사람들의 찬반 논쟁이나 댓글을 유도하는 신랄한 평이나 도발적인 질문을 던지세요.
  - 5줄 (해시태그): 첫 번째 해시태그는 무조건 1줄(제목)에 사용했던 [최우선 핵심 키워드]로 지정하고, 이어서 본문 핵심 키워드를 활용해 총 3~4개의 해시태그를 띄어쓰기로 나열하세요. (예: #제목키워드 #본문키워드1)
3. 인사말이나 구구절절한 기사 요약은 절대 쓰지 마세요.`;

  let finalBasePrompt = baseFeedPrompt || (provider === 'gemma-4-31b' ? proFallbackPrompt : fallbackPrompt);
