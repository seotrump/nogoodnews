  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="p-4 bg-gray-50 rounded-lg border border-gray-100">
        <h2 className="text-sm font-semibold text-gray-500 mb-1">{t('accountEmail')}</h2>
      ...
      <div className="pt-4 border-t border-gray-100">
        <label className="block text-sm font-semibold mb-2 text-gray-700">{t('language')}</label>
        <select 
          value={selectedLocale}
          onChange={(e) => setSelectedLocale(e.target.value)}
          disabled={isPending || isSaving}
          className="w-full border border-gray-200 p-3 rounded-lg focus:ring-2 focus:ring-black focus:outline-none bg-white cursor-pointer disabled:opacity-50"
        >
